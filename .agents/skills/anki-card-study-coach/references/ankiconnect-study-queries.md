...
4. `cardsInfo`
5. `getReviewsOfCards`
6. `scripts/select_study_cards.py`
...
```bash
python3 scripts/ankiconnect_request.py --action getActiveProfile --pretty
```

查询牌组里的卡片：

```bash
python3 scripts/ankiconnect_request.py \
  --action findCards \
...
```bash
python3 scripts/select_study_cards.py --deck "Vocabulary" --limit 8 --seed 20260328 --pretty
```

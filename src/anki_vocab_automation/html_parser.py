...
class HTMLParser:
    ...
    def parse_collins_response_with_dual_pronunciation(
        self, response_data: Dict[str, Any], original_word: str
        ...
        try:
            ...
            card = VocabularyCard(
                ...
                audio_source="Dictionary" if main_audio_url else "",
                british_audio_source=(
                    "Dictionary" if dual_pronunciation_data.get("british_audio_url", main_audio_url) else ""
                ),
                american_audio_source="Dictionary" if dual_pronunciation_data.get("american_audio_url", "") else "",
                source="collins",
            )
    ...
    def parse_collins_response(self, response_data: Dict[str, Any], original_word: str) -> Optional[VocabularyCard]:
        ...
        try:
            ...
            card = VocabularyCard(
                ...
                part_of_speech=part_of_speech,
                original_word=original_word,
                audio_source="Dictionary" if audio_url else "",
                source="collins",
            )
    ...
    def _extract_definition(self) -> str:
        """提取定义"""
        # 使用更具体的选择器
        selectors = [".def", ".definition", ".sense .def", "div.sense .def", ".collins-definition", ".meaning"]
    ...
    def _extract_example_from_soup(self, soup: BeautifulSoup) -> str:
        ...
        # 次优选择器（可能包含引用或例句）
        medium_priority_selectors = [".cit", ".citation", ".quotation", ".sense .cit", ".illus", ".sample"]
    ...
    def _clean_pronunciation(self, pronunciation: str) -> str:
        ...
        pronunciation = re.sub(
            r"[^\w\s/ˈˌæɑɒɔɪʊʌɛɜɝəɞɘɵɐɯɨʉɥɪeɛoʔθðʃʒŋtʃdʒjwrhʰʱ̃ː̩̯̍̆̈̂̌̄̀́̋̏̌̑̒̓̔̕̚ʷʲʳⁿᵗˢᵖ]", "", pronunciation
        )
    ...
    def _extract_part_of_speech(self) -> str:
        """提取词性"""
        # 使用更具体的选择器
        selectors = [".pos", "span.pos", ".part-of-speech", ".grammar", ".wordtype"]

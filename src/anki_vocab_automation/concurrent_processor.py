...
class ConcurrentProcessor:
    ...
    def __init__(self, 
        ...
        logger.info(f"并发处理器初始化: max_workers={max_workers}, "
    ...
    def _process_single_word(self, 
                           word: str, 
                           processor_func: Callable[[str], Optional[VocabularyCard]],
                           word_index: int,
                           total_words: int) -> ProcessingResult:
        """
        处理单个单词（带重试和错误处理）
        ...
        if not is_valid:
            logger.warning(f"[线程{thread_id}] 单词 '{word}' 验证失败: {error_msg}")
        ...
        # 清理输入
        clean_word = sanitize_word_input(word)
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                logger.info(f"[线程{thread_id}] 处理单词 '{clean_word}' "
                           f"({word_index + 1}/{total_words}) - 尝试 {attempt + 1}")
                
                # 调用处理函数
                result = processor_func(clean_word)
                
                processing_time = time.time() - start_time
                
                if result is not None:
                    logger.info(f"[线程{thread_id}] 成功处理 '{clean_word}' "
                ...
                last_error = str(e)
                logger.warning(f"[线程{thread_id}] 处理 '{clean_word}' 失败 "
                              f"(尝试 {attempt + 1}): {last_error}")
            ...
            for future in as_completed(future_to_word, timeout=self.timeout_per_word * len(words)):
                try:
                    ...
                    if completed_count % 5 == 0 or completed_count == len(words):
                        logger.info(f"进度: {completed_count}/{len(words)} "
    ...
    def process_words_with_fallback(self,
                                  words: List[str],
                                  primary_processor: Callable[[str], Optional[VocabularyCard]],
                                  fallback_processor: Optional[Callable[[str], Optional[VocabularyCard]]] = None,
                                  progress_callback: Optional[Callable[[int, int, ProcessingResult], None]] = None
                                  ) -> Tuple[List[ProcessingResult], ProcessingStats]:
        """
        使用主处理器和备用处理器进行批量处理
        ...
        Returns:
            (结果列表, 统计信息)
        """
        def combined_processor(word: str) -> Optional[VocabularyCard]:
            """组合处理器"""
            # 首先尝试主处理器
            try:
                result = primary_processor(word)
                if result is not None:
                    return result
            except Exception as e:
            ...
            if fallback_processor is not None:
                try:
                    logger.info(f"使用备用处理器处理 '{word}'")
                    return fallback_processor(word)
                except Exception as e:
...
# 便利函数
def process_words_concurrently(words: List[str],
                             processor_func: Callable[[str], Optional[VocabularyCard]],
                             max_workers: int = 4,
                             rate_limit: Optional[float] = None,
                             show_progress: bool = True) -> Tuple[List[ProcessingResult], ProcessingStats]:
    """
    并发处理单词的便利函数

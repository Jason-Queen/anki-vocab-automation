...
class CollinsAPI:
    ...
    def __init__(self, api_key: str = COLLINS_API_KEY):
        ...
        if api_key:
            self.headers = {"accessKey": api_key, "User-Agent": USER_AGENT}
        else:
    ...
    def _get_dual_pronunciation_data(self, word: str) -> Dict[str, Any]:
        ...
        Returns:
            包含双重发音信息的字典
        """
        result = {"british_data": None, "american_data": None, "pronunciations": []}
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if e.response.status_code == 404:
                    ...
                    logger.warning(
                        f"API请求失败 - {word} (词典: {dictionary}, 尝试 {attempt + 1}/{MAX_RETRIES}): {str(e)}"
                    )
